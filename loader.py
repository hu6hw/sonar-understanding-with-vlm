...
TURNTABLE_MATERIAL_OBJECT_MAPPING = {
...
TURNTABLE_OBJECT_CLASS_MAPPING = {
    "bottle": ["brown-glass-bottle", "plastic-bottle", "metal-bottle", "glass-bottle", "potion-glass-bottle"],
    "can": ["can"],
    "carton": ["drink-carton"],
    "box": ["metal-box"],
    "bidon": ["plastic-bidon"],
...
TURNTABLE_INDIVIDUAL_OBJECT_CLASSES = [
...
WATERTANK_CROPPED_DATA_FOLDER = os.path.join(pathlib.Path(__file__).parent.absolute() ,"data/watertank-cropped")

VALID_DATASETS = ["turntable", "watertank"]
VALID_CROPS = ["platform", "object"]
VALID_TASKS = ["object_classes", "individual_objects", "materials"]

SPLIT_SEED = 42

def load_debris_dataset(dataset, task="object_classes", crop="platform", image_size = (96, 96), split=None, add_channels_dim=True):
    """
    ...
    if dataset == "watertank":
        if task == "object_classes":
            object_images, object_labels = load_watertank_objects(WATERTANK_OBJECT_CLASSES, image_size)

            return make_split(object_images, object_labels, split)

    if dataset == "turntable":
        if task == "object_classes":
            object_images, object_labels = load_turntable_objects(TURNTABLE_OBJECT_CLASS_MAPPING, TURNTABLE_OBJECT_CLASSES, image_size, crop)

            return make_split(object_images, object_labels, split)

        if task == "individual_objects":
            object_images, object_labels = load_turntable_objects({x: x for x in TURNTABLE_INDIVIDUAL_OBJECT_CLASSES},
                                                                TURNTABLE_INDIVIDUAL_OBJECT_CLASSES, image_size, crop)

            return make_split(object_images, object_labels, split)

        if task == "materials":
            object_images, object_labels = load_turntable_objects(TURNTABLE_MATERIAL_OBJECT_MAPPING, TURNTABLE_MATERIAL_CLASSES, image_size, crop)
...
def class_names(dataset, task):
    ...
    if dataset == "turntable":
        if task == "object_classes":
            return TURNTABLE_OBJECT_CLASSES
        elif task == "materials":
            return TURNTABLE_MATERIAL_CLASSES
        elif task == "individual_objects":
            return TURNTABLE_INDIVIDUAL_OBJECT_CLASSES

    if dataset == "watertank":
        if task == "object_classes":
            return WATERTANK_OBJECT_CLASSES

    raise ValueError("Unknown dataset {} or task {}".format(dataset, task))
...
def load_turntable_individual_object(object_name, size, crop="platform"):
    """
        Loads all images for a particular object in the turntable dataset.
    """
...
def load_watertank_objects(class_names, size):
    object_images, object_labels = [], []

    for class_name in class_names:
        obj_images = load_watertank_individual_object(class_name, size)
        label = class_names.index(class_name)

        object_images.append(obj_images)
        object_labels.append([label] * obj_images.shape[0])

    return object_images, object_labels

def load_watertank_individual_object(object_name, size):
    """
        Loads all images for a particular object in the watertank dataset.
    """
    glob_pattern = "{}/{}/*.png".format(WATERTANK_CROPPED_DATA_FOLDER, object_name)
